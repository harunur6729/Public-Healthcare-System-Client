...
const AddDoctor = () => {
    ...
    useEffect(() => {
        const fetchSpecialties = async () => {
            try {
                const res = await axios.get('https://public-healthcare-system-server.vercel.app/appointmentSpecialty');
    ...
    const saveUser = async (doctor) => {
        const response = await fetch('https://public-healthcare-system-server.vercel.app/users', {
            method: 'POST',
    ...
    const UpdateDoctorUser = async (doctor) => {
        try {
            const response = await fetch('https://public-healthcare-system-server.vercel.app/v2/updateAppointmentOptions', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
